...
def _split_sentences(text: str) -> list[str]:
    ...
    if not text:
        return []
    # simple sentence split (kept lightweight/offline)
...
def _looks_like_tax_id(s: str) -> bool:
    ...
    if not s:
        return False
    # Very loose check: alnum length range
...
def build_vendor_insights(
    ...
    # Manufacture date plausibility (if provided)
    if manufacture_date:
        try:
            # mysql connector often returns date/datetime; handle both
            if hasattr(manufacture_date, "date"):
                dt = manufacture_date if isinstance(manufacture_date, datetime) else datetime.combine(manufacture_date, datetime.min.time())
            else:
    ...
    # Suspicious keywords in free-text details (lightweight “AI-like” heuristic)
    ...
    # If score is high, elevate recommendations
